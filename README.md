...
<!-- tool-catalog:start — generated by `tsx scripts/print-tool-catalog.ts` from ALL_TOOLS; full reference in docs/tools.md -->
| # | Name | Title | Legacy | Description (first line) |
|---|------|-------|--------|--------------------------|
| 1 | `search_datasets` | Search datasets | yes | Search for datasets on data.gouv.fr by keywords. |
| 2 | `search_organizations` | Search organizations | yes | Find publishing organizations on data.gouv.fr (who publishes datasets and |
| 3 | `search_dataservices` | Search third-party APIs | yes | Search for third-party APIs (dataservices) on data.gouv.fr by keywords. |
| 4 | `get_dataservice_info` | Get third-party API info | yes | Get detailed metadata about a specific third-party API (dataservice). |
| 5 | `get_dataservice_openapi_spec` | Get third-party API OpenAPI spec | yes | Fetch and summarize the OpenAPI/Swagger spec for a third-party API (dataservice). |
| 6 | `get_dataset_info` | Get dataset info | yes | Get detailed metadata about a specific dataset. |
| 7 | `list_dataset_resources` | List dataset resources | yes | List all resources (files) in a dataset with their metadata. |
| 8 | `get_resource_info` | Get resource info | yes | Get detailed information about a specific resource (file). |
| 9 | `query_resource_data` | Query resource data | yes | Query tabular data from a resource via the Tabular API (no download needed). |
| 10 | `get_metrics` | Get usage metrics | yes | Get usage metrics (visits, downloads) for a dataset or resource. |
| 11 | `check_resource_availability` | Check resource availability | no | Check whether a resource's file URL is actually reachable before spending calls on it. |
| 12 | `get_dataset_resources_summary` | Get dataset resources summary | no | One-call overview of a dataset and all its resources, with the best way to access each one. |
| 13 | `get_resource_schema` | Get resource schema | no | Return the columns (name, type) and, when known, the row count of any queryable resource. |
| 14 | `get_reuse_info` | Get reuse info | no | Get the details of one reuse: description, type, topic, tags, publisher and the list of |
| 15 | `list_high_value_datasets` | List high value datasets (HVD) | no | Browse the High Value Datasets (HVD): datasets flagged under the EU Open Data Directive as |
| 16 | `list_topics` | List topics | no | List curated topics (thematic collections of datasets and reuses maintained on data.gouv.fr), |
| 17 | `get_topic` | Get topic | no | Get a topic (curated collection) with its description and the datasets it groups. |
| 18 | `preview_resource` | Preview resource | no | Safely look inside any resource, whatever its format, with hard size caps. |
| 19 | `query_resource` | Query resource | no | Query the rows of any tabular resource with filters, sort and pagination, whatever the format. |
| 20 | `search_reuses` | Search reuses | no | Find reuses — applications, visualisations, articles, APIs — built on data.gouv.fr datasets. |
| 21 | `suggest` | Suggest (autocomplete) | no | Cheap autocomplete across datasets, organizations, tags, spatial zones and formats. |
<!-- tool-catalog:end -->

Regenerate the table with `tsx scripts/print-tool-catalog.ts`. Full per-tool reference with output shapes: [docs/tools.md](docs/tools.md).
...
```mermaid
flowchart BT
    core[src/core<br/>config · errors · cache · http]
    clients[src/clients<br/>datagouv v1/v2 · tabular · metrics · crawler · schema]
    formats[src/formats<br/>capability · accessors · engines]
    tools[src/tools<br/>ALL_TOOLS · registry]
    server[src/server<br/>stdio + Hono HTTP /mcp /health]
...
pnpm test:live                           # real data.gouv.fr calls (DATAGOUV_LIVE=1)
