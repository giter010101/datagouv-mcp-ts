...
| **D — tests & evidence** | Fable → Composer | `tests/helpers/**`, `tests/live/**`, `scripts/evidence.ts`, `scripts/record-fixtures.ts`, `scripts/conformance.ts`, `docs/evidence/**`, `vitest.config.ts` | M0 | **mostly done** — harness + 42 API fixtures + 8 file fixtures; 99 offline tests green; **1 live evidence report** (`search_datasets-live.md`); contract tests missing; 20 tools without live evidence |
