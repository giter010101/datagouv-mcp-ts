...
| **B — formats** | Grok (this run) | `src/formats/**`, `tests/fixtures/files/**`, `tests/unit/formats/**`, `.agent/exec-plans/active/ws-b-formats.md` | M0 (uses A's interfaces via fakes) | **in_progress → review** — detector/parsers/engines + **accessors registered**; `defaultAccessors` / `openResource` exported; offline `tests/unit/formats` green |
