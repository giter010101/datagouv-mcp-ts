...
| `check_resource_availability` | Check resource availability | not yet | `resource_id` |
| `get_dataservice_info` | Get third-party API info | not yet | `dataservice_id` |
| `get_dataservice_openapi_spec` | Get third-party API OpenAPI spec | not yet | `dataservice_id` |
| `get_dataset_info` | Get dataset info | not yet | `dataset_id` |
| `get_dataset_resources_summary` | Get dataset resources summary | not yet | `dataset_id` |
| `get_metrics` | Get usage metrics | not yet | — |
| `get_resource_info` | Get resource info | not yet | `resource_id` |
| `get_resource_schema` | Get resource schema | not yet | `resource_id` |
| `get_reuse_info` | Get reuse info | not yet | `reuse_id` |
| `get_topic` | Get topic | not yet | `topic_id` |
| `list_dataset_resources` | List dataset resources | not yet | `dataset_id` |
| `list_high_value_datasets` | List high value datasets (HVD) | not yet | — |
| `list_topics` | List topics | not yet | — |
| `preview_resource` | Preview resource | not yet | `resource_id` |
| `query_resource` | Query resource | not yet | `resource_id` |
| `query_resource_data` | Query resource data | not yet | `resource_id` |
| `search_dataservices` | Search third-party APIs | not yet | `query` |
| `search_organizations` | Search organizations | not yet | — |
| `search_reuses` | Search reuses | not yet | — |
| `suggest` | Suggest (autocomplete) | not yet | `query` |
...
| `organization` | string | no | — | Facet: organization ID (from search_organizations) to restrict results to one publisher. |
| `tag` | string | no | — | Facet: exact tag slug (e.g. 'transport'). |
| `license` | string | no | — | Facet: license ID (e.g. 'lov2', 'odc-odbl'). |
| `format` | string | no | — | Facet: resource format present in the dataset (e.g. 'csv', 'geojson', 'parquet'). |
| `badge` | string | no | — | Facet: dataset badge, e.g. 'hvd' (High Value Dataset, EU regulation) or 'spd' (public service data). |
| `geozone` | string | no | — | Facet: spatial zone ID (e.g. 'fr:commune:75056'); find IDs with suggest(kind='zone'). |
| `granularity` | string | no | — | Facet: spatial granularity (e.g. 'fr:commune', 'fr:departement', 'country'). |
| `schema` | string | no | — | Facet: schema.data.gouv.fr schema name (e.g. 'etalab/schema-irve-statique'). |
| `topic` | string | no | — | Facet: topic ID (from list_topics). |

### `check_resource_availability`

**Check resource availability** — `src/tools/check-resource-availability.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Get third-party API info** — `src/tools/get-dataservice-info.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Get third-party API OpenAPI spec** — `src/tools/get-dataservice-openapi-spec.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Get dataset info** — `src/tools/get-dataset-info.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Get dataset resources summary** — `src/tools/get-dataset-resources-summary.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Get usage metrics** — `src/tools/get-metrics.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Get resource info** — `src/tools/get-resource-info.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Get resource schema** — `src/tools/get-resource-schema.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Get reuse info** — `src/tools/reuses.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Get topic** — `src/tools/topics.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**List dataset resources** — `src/tools/list-dataset-resources.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**List high value datasets (HVD)** — `src/tools/list-high-value-datasets.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**List topics** — `src/tools/topics.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Preview resource** — `src/tools/preview-resource.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Query resource** — `src/tools/query-resource.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Query resource data** — `src/tools/query-resource-data.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Search third-party APIs** — `src/tools/search-dataservices.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Search organizations** — `src/tools/search-organizations.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Search reuses** — `src/tools/reuses.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
...
**Suggest (autocomplete)** — `src/tools/suggest.ts` (in `src/tools` but not yet in `ALL_TOOLS`)
