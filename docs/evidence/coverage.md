...
Offline reports were produced on 2026-09-03 by `EVIDENCE_WRITE=1 pnpm exec vitest run tests/e2e/all-tools-offline.test.ts` (in-process MCP + recorded fixtures). Live stdio reports (`*-live.md`) were produced with `pnpm exec tsx scripts/evidence-live.ts` (`node dist/index.js` JSON-RPC against production).
